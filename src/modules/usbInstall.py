...
class Endpoint:
    def __init__(self, config, direction):
        def check_endpoint(direction):
            return (
                lambda ep: usb.util.endpoint_direction(ep.bEndpointAddress) == direction
            )

        self.ep: usb.core.Endpoint = usb.util.find_descriptor(
            config, custom_match=check_endpoint(direction)
        )
...
class SwitchUsb(GObject.GObject):
    __gsignals__ = {
        ...
        "error": (GObject.SIGNAL_RUN_FIRST, None, tuple()),
    }
    ...
    def validate_roms(self, roms: list[str]):
        ...
        roms_length = 0
        for file in roms:
            if os.path.isfile(file) is False or os.path.splitext(file)[1] not in [
                ".nsp",
                ".xci",
            ]:
